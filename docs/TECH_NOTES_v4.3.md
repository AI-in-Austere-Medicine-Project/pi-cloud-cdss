...
- **Offline regression suite: 1,335 tests, ~12s** (`server/run_unit_tests.sh`) — no network, no API key, no ChromaDB. Runs on a clean checkout. This is the gate for every change to the deterministic layer. `test_vent_module.py` contributes 143 of them
