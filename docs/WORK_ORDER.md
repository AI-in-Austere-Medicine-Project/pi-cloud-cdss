...
**Execution order (owner, 2026-09-26):** A0 → A1b → D5 → D6. Same rules; stop for review on each.
...
### D5: distillation dataset (added 2026-09-26; after A0 and A1b are merged and deployed)

Script: `tools/build_distill_dataset.py`. Source: session logs and benchmark run outputs.

Include only answers that:
- **(a)** were generated by a CLOUD model. Check the served-model field. Exclude anything qwen wrote, including local fallback.
- **(b)** pass every current deterministic check when REPLAYED through today's code, not the check that ran at the time.
- **(c)** were not fixed cards. No model call means nothing to learn.
- **(d)** are single-patient turns with no history from a prior patient.

Rebuild each prompt by calling the live pipeline's own prompt-construction path. Never reassemble it by hand. The training prompt and the serving prompt must be byte-identical in shape.

Output: `train.jsonl` and `valid.jsonl`, split 90/10 by scenario, not by turn. The format is exactly the messages format at `~/edgecdss-train/data-dryrun/` on the Mac (system / user / assistant triples).

Report: the count per source, the count excluded per rule, and the ten most common scenario types.

**Do not train.**

### D6: training toolchain (added 2026-09-26; on the Mac at `~/edgecdss-train`, may be built in parallel with D5)

A Makefile with pinned versions:
- **`requirements.txt`** pins the exact working versions of mlx-lm, transformers, huggingface_hub (< 2.0), tokenizers and mlx. Read them from the current venv with `pip freeze`.
- **`make train ADAPTER=name`:** `mlx_lm.lora` on `mlx-community/Qwen2.5-3B-Instruct-4bit`. `--iters` and `--learning-rate` are variables; the defaults are 600 and 1e-4.
- **`make fuse`:** `mlx_lm.fuse` against `Qwen/Qwen2.5-3B-Instruct` with `--dequantize`. Then copy `tokenizer.json`, `tokenizer_config.json`, `vocab.json` and `merges.txt` from the base snapshot over the fused folder (the transformers/MLX tokenizer-config mismatch).
- **`make gguf`:** llama.cpp `convert_hf_to_gguf.py` to f16, then `llama-quantize` to Q4_K_M. Never ship f16.
- **`make ship`:**
  - scp the Q4 GGUF to the Jetson;
  - write a Modelfile and `ollama create edgecdss-<name>`;
  - run the ketamine probe ON the Jetson with a single-line prompt and print the eval rate;
  - refuse to run if the Jetson has less than 3 GB free.
- **`make bench`:** point a second uvicorn at the new model on :8001, run the 30-set and `run_tests.sh`, and append to `LOCAL_LLM_BENCHMARK.md` with the adapter name.

Also write `~/edgecdss-train/README.md` with the three gotchas:
1. Never `-U` the foundation libraries.
2. Copy the tokenizer files after fuse.
3. Quantize on the Mac. Run probes on the Jetson directly, not through multi-line ssh quotes.
