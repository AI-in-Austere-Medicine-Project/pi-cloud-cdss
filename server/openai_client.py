...
def build_full_query_history(query: str, conversation_history: Optional[list] = None) -> tuple[str, str]:
    """Return prior_queries and full_query_history for deterministic gates."""
    prior_queries = ""
    if conversation_history:
        parts = []
        for turn in conversation_history[-8:]:
...
**DON'T**
- No steroids. No albumin. Avoid routine hyperventilation unless herniation.

**TLDR**
- Severe TBI: airway, fluid/perfusion, ICP monitoring, evacuate.
...
def query_with_rag(query: str, chromadb_client, voice_mode: bool = False,
    ...
    try:
        ...
        if is_cico_query(full_query_history):
            print("🛑 CICO PRE-GATE")
            return {
        ...
        if asks_for_wpw_contraindicated_drug(full_query_history):
        ...
        # Step 2d: TXA in infection/sepsis hard safety block
        if (
            has_infection_context(full_query_history)
            and asks_for_txa(full_query_history)
            and not has_clear_hemorrhage(full_query_history)
        ):
        ...
        if (
            looks_like_sepsis(full_query_history)
            and asks_for_dcr_or_hemostatic_resus(full_query_history)
            and not has_clear_hemorrhage(full_query_history)
        ):
        ...
        if (
            looks_like_hemorrhagic_shock(full_query_history)
            and not looks_like_sepsis(full_query_history)
        ):
        ...
        if looks_like_sepsis(full_query_history) and not has_clear_hemorrhage(full_query_history):
        ...
        general_response = build_general_case_response(full_query_history)
        if general_response:
        ...
        # Step 2j: Pediatric ketamine route — deterministic, before standard pre-gate
        if (
            ...
            ped_ket_response = build_pediatric_ketamine_route_response(patient_ctx)
            if ped_ket_response:
                print("💊 PED KETAMINE ROUTE PRE-GATE")
                return {
                    "response": ped_ket_response,
                    "sources": [],
                    "source_mode": "DETERMINISTIC_PRE_GATE",
                    "validator_result": "SAFE",
                    "validator_issues": [],
        ...
        if is_rsi_or_post_intubation_context(full_query_history):
            rsi_response = build_rsi_response(patient_ctx, full_query_history)
            if rsi_response:
                print("💉 RSI PRE-GATE")
                return {
